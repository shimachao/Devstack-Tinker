...
### 2. 在虚拟机中安装程序
#### 2.1 __传输程序安装包到虚拟机中__

在宿主机中通过 scp 命令将流体模拟程序传输到虚拟机中。
```shell
$ scp -P 2222 -r ~/fluid-bin32 chao@127.0.0.1:~/
```
因为前面我们将宿主机的 TCP 2222号端口转向到虚拟机的22号端口了，所以这里我们连接宿主机本地 IP 的2222号端口就行。
#### 2.2 __安装__
...
保持主机上的 synthesis_server 开启。叫手机连接到无线局域网环境中，运行手机上的 Cloudlet APP。你会看到主机上动态合成了一个虚拟机实例。手机这边 Cloudlet APP 弹出一个流体模拟程序。并且该流体程序连接上了主机上虚拟机实例中的服务端。
